...
# fake points (0) added in the first three lines, numerical reason
    dataAM=np.loadtxt('/home/x_konio/AM.txt')
...
########### band-gap array #############
    ...
    BgCIS=0.87 #This must be inputted when starting the program
    BgMat = BgCIS
    # thickness nm
    tkArr=np.array([10, 50, 100, 200, 500, 1000, 1500])
    ...
    for j in range(len(bgArrReal)):
        ...
        for i in range(len(tkArr)):
            ...
            maxEff=calEff(tkArr[i],bgArrReal[j],bgShift[j],0,0,1,fileName[j], 0.7)
            print(maxEff)
